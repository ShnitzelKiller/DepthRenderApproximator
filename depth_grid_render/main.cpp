...
std::shared_ptr<XMLElement> buildScene(std::string envmap, float alpha, const Eigen::Vector3f &camOrigin, const Eigen::Vector3f &planeOrigin, const Eigen::Vector2f &planeScale, std::string scene_version = "0.6.0", bool pointlight = false, Eigen::Vector3d light_pos = Eigen::Vector3d(), std::string meshPath="", std::string meshTexture="") {
    using namespace std;
    ostringstream eye;
...
int main(int argc, char** argv) {
    ...
    auto scene = buildScene(envmap, alpha, eye, planeOrigin, planeScale, scene_version, light, Eigen::Vector3d(lightX, lightY, lightZ), mesh_path);
    std::ofstream sceneof(scene_path);
    scene->SaveXML(sceneof);
    sceneof.close();
    auto texscene = buildScene(envmap, alpha, eye, planeOrigin, planeScale, scene_version, light, Eigen::Vector3d(lightX, lightY, lightZ), mesh_path, texture_image);
    std::ofstream texsceneof(textured_scene_path);
    texscene->SaveXML(texsceneof);
    texsceneof.close();
    auto woscene = buildScene(envmap, alpha, eye, planeOrigin, planeScale, scene_version, light, Eigen::Vector3d(lightX, lightY, lightZ), meshwo_path, texture_image);
